...
def grid_search(dataset, save_path='grid_search_results.json'):
	"""
	Esegue una grid search degli iperparametri basata sui migliori parametri trovati.
	...
	# Definisci la griglia di ricerca centrata sui parametri attuali del main
	# Architettura attuale: 512-512-128, Mish, Dropout 0.1/0.2, LR 0.1, weight_decay 1e-5
	param_grid = {
		'hidden_layer_1': [256, 512],  # variazioni attorno a 512
		'hidden_layer_2': [256, 512],  # variazioni attorno a 512
		'hidden_layer_3': [64, 128, 256],  # variazioni attorno a 128
		'dropout_rate_1': [0.1, 0.15],  # variazioni attorno a 0.1
		'dropout_rate_2': [0.15, 0.2],  # variazioni attorno a 0.2
		'learning_rate': [0.05, 0.1],  # variazioni attorno a 0.1
		'batch_size': [256],  # solo il valore ottimale
		'epochs': [300],  # solo il valore ottimale
		'weight_decay': [1e-5, 1e-4]  # variazioni attorno a 1e-5
	}
	...
	for i, params in enumerate(combinations):
		...
		try:
			# Crea modello custom con i parametri scelti (usa Mish come nel main)
...
def train_model_custom(model, dataset, epochs=200, batch_size=256, learning_rate=0.01, weight_decay=1e-5):
	"""Versione modificata di train_model con weight_decay configurabile e CosineAnnealingLR"""
	...
	# Usa CosineAnnealingLR come nel main
	...
	for epoch in range(epochs):
		...
		scheduler.step()  # Step dopo ogni epoca per CosineAnnealingLR
...
if __name__ == "__main__":
	...
	if len(sys.argv) < 3:
		print("Usage: python grid_search.py <train_file> <test_file>")
		sys.exit(1)
	...
	best_params, best_score = grid_search(
		dataset, 
		save_path='grid_search_results.json'
	)
