...
class MLPControllerNode(Node):
    ...
    def _load_model(self):
        ...
        if self.njoints == 3:
            ...
            print("Input size for 3 joints:", input_size)
        elif self.njoints == 4:
    ...
    def target_callback(self, msg):
        """Callback for target position messages - makes prediction and prints difference."""
        self.target = [msg.point.x, msg.point.y, msg.point.z]
        
        # Check if we have all necessary data
        if self.q is None:
            self.get_logger().warn("No joint state received yet")
            self.q = [0.0] * self.njoints  # Default to zeros
        
        # Update end-effector position
        if not self._update_ee_position():
            self.get_logger().warn("Could not get EE position from TF")
            return
        ...
        self.get_logger().info(f"Target received: [{self.target[0]:.4f}, {self.target[1]:.4f}, {self.target[2]:.4f}]")
        self.get_logger().info(f"Current EE pos:  [{self.x[0]:.4f}, {self.x[1]:.4f}, {self.x[2]:.4f}]")
        self.get_logger().info(f"Distance to target: {dt_norm:.4f} m")
        self.get_logger().info(f"Current q: [{', '.join([f'{qi:.4f}' for qi in self.q])}]")
        
        # Normalize displacement for prediction
        ...
        self.get_logger().info(f"Predicted dq (raw): [{', '.join([f'{dqi:.6f}' for dqi in dq])}]")
        
        # Apply constraints
        dq_clipped = self._clip_dq(dq)
        
        self.get_logger().info(f"Predicted dq (clipped): [{', '.join([f'{dqi:.6f}' for dqi in dq_clipped])}]")
        
        # Compute target joint angles
        q_target = np.array(self.q) + dq_clipped
        
        # Apply joint limits
        q_target = self._apply_joint_limits(q_target)
        
        self.get_logger().info(f"Target q: [{', '.join([f'{qi:.4f}' for qi in q_target])}]")
